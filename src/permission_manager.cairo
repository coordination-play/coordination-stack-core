...
mod PermissionManager {
    ...
    #[external(v0)]
    impl PermissionManager of super::IPermissionManager<ContractState> {
        //
        // Getters
        //
        fn is_granted(self: @ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) -> bool {
        ...
        fn grant(ref self: ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) {
        ...
        fn revoke(ref self: ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) {
            let has_root_permission = self._root_auth(where);
            assert (has_root_permission, 'UNAUTHORISED');
        ...
        fn apply_single_target_permission(ref self: ContractState, where: ContractAddress, permissions: Array::<SingleTargetPermission>) {
            let has_root_permission = self._root_auth(where);
            assert (has_root_permission, 'UNAUTHORISED');
        ...
        fn apply_multiple_target_permission(ref self: ContractState, permissions: Array::<MultiTargetPermission>){
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _root_auth(ref self: ContractState, where: ContractAddress) -> bool {
            let caller = get_caller_address();
            let current_contract = get_contract_address();
        ...
        fn _grant(ref self: ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) {
        ...
        fn _revoke(ref self: ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) {
            let perm_hash: felt252 = InternalImpl::_permission_hash(@self, where, who, permission_id);
            let current_flag = self._permissions_hashed.read(perm_hash);
        ...
        fn _permission_hash(self: @ContractState, where: ContractAddress, who: ContractAddress, permission_id: felt252) -> felt252 {
